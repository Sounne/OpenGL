...
bool CreateFramebuffer(int width, int height)
{
	...
	a = new Test(8);		
	//b = std::move(new Test(8));	// b = a
	//b = std::move(*a);		// ref(*a)
	b = *a;
	b = makeTemp(6);

	// c++ 2003, 2 constructeur (dont 2 malloc), 1 copie, 1 destructeur avec free()
	// C++ 2011, 1 constructeur (dont 1 malloc), 1 move constructor, 1 destructeur qui ne fait rien
	Test c = *a;
	Test d(*a);
...
void Initialize()
{
	...
	BasicProgram.LoadShader(GL_VERTEX_SHADER, "basicPostProcess.vs");
	// extension .f, .fs, .frag, *.glsl ou autre
	BasicProgram.LoadShader(GL_FRAGMENT_SHADER, "basicPostProcess.fs");
	BasicProgram.Create();
